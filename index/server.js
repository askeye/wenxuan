...
//购物车小数字
app.get('/car_num', (req, res) => {
    connection.query(`select count(admin) as nums from b_car where admin='${req.query.user}'`, function(err, result) {
        res.json(result)
    })
});
...
app.get('/collection', (req, res) => {
    connection.query(`select * from b_collection where admin='${req.query.admin}'`, function(err, result) {
        const obj = {
            status: 200,
            data: result
        }
...
app.get('/addcollection', (req, res) => {
    connection.query(`insert into b_collection (title,newprice,oldprice,imgurl,book_id,admin,publish)values('${req.query.title}',${req.query.newprice},${req.query.oldprice},'${req.query.imgurl}',${req.query.book_id},'${req.query.admin}','${req.query.publish}')`, function(err, result) {
        const obj = {
            status: 200
        }
        res.json(obj)
